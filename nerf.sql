...
#nerf taq
UPDATE creature_template SET MinLevelHealth = ROUND(MinLevelHealth / 10), MaxLevelHealth = ROUND(MaxLevelHealth / 10), MinLevelMana = ROUND(MinLevelMana / 10), MaxLevelMana = ROUND(MaxLevelMana / 10), MinMeleeDmg = ROUND(MinMeleeDmg / 10), MaxMeleeDmg = ROUND(MaxMeleeDmg / 10), MinRangedDmg = ROUND(MinRangedDmg / 10), MaxRangedDmg = ROUND(MaxRangedDmg / 10) WHERE entry IN (15229,15230,15233,15235,15236,15240,15246,15247,15249,15250,15252,15262,15263,15264,15275,15276,15277,15299,15300,15311,15312,15316,15317,15378,15379,15380,15502,15503,15504,15509,15510,15511,15516,15543,15544,15589,15727,15896,15957,15963,15984,15517);
